...
**Injury adjustment:** After the base load is computed, we optionally **shrink the denominator** for absences we treat as excused. Three signals feed it. `injuryReportWeeks` and `reserveWeeks` are unioned as week sets and that union is intersected with the weeks actually missed — not combined by `max()` — see the rule below. `seasonEndingAbsenceGames`, the pre-2016 fallback, is era-exclusive with `reserveWeeks` and is still combined with the result by `max()`, not intersection.
...
- `reserveWeeks`: distinct weeks a player's own weekly roster status reads `RES`/`RSR`/`PUP`/`NON` (`src/lib/reserveWeeks.ts`), sourced from nflverse's `roster_weekly` release and available from **2016** on (`FIRST_RESERVE_SEASON`). This is the direct signal `seasonEndingAbsenceGames` was approximating: a player who returns from IR mid-season, like Derwin James in 2019, appears every week in `snap_counts` **after** his return and reads as healthy under the season-ending heuristic — zero excusal for a real IR stint. Reserve status catches him because it is recorded directly, not inferred from a gap in play.

`seasonEndingAbsenceGames` and `reserveWeeks` are **era-exclusive**: `update-data.ts` writes `reserveWeeks` for 2016+ seasons and `seasonEndingAbsenceGames` only for 2013–2015, never both on the same season (see `Season` in `docs/datamodel.md`). 2013–2015 roster data was tested and rejected as a `reserveWeeks` source — under half the players who reach IR in a real season show any reserve week, and the weeks recorded skew toward the start of the season rather than the end (Tyler Eifert's 2014 dislocated-elbow year, out from week 1, shows a single reserve row **at week 1**). `FIRST_RESERVE_SEASON` is a coverage floor in the same family as `FIRST_SNAP_SEASON` and `FIRST_INJURY_SEASON`.

Let `missedWeeks` be the set of team weeks the player did not appear in, over the schedule the **rest rule** (§1.6) leaves behind — a rested finale is erased there, so excusing it here as well would discount one absence twice. The excusal is

```
excusedGames = | missedWeeks ∩ ( injuryWeeks ∪ reserveWeeks ) |
```

and it is written to each `Season` as `excusedGames` (`src/lib/absenceWeeks.ts`). Two set operations, because two different questions are being asked. The **union** answers _was he hurt in this week_: the injury report and the reserve list are not alternative estimates of one absence, so `max()` was never the right way to combine them. A player placed on injured reserve is **removed** from the weekly injury report, so the two feeds describe consecutive halves of a single absence. Ronnie Stanley 2021 is the shape — report weeks 1–6, reserve weeks 7–18 (week 8 is the bye), zero overlap: `max()` of the counts saw 11 of the 16 games he lost, the union sees all 16. The **intersection** with `missedWeeks` answers the other question, _did that week cost a game_: being listed on the injury report does not mean sitting out, so `injuryReportWeeks` counts weeks spent on the report rather than games lost, and intersecting removes the games it would otherwise forgive that were never missed.

Two properties follow. The rule is **monotone in evidence**: another documented week can only widen the union, so it can never forgive less, and no week without documented injury behind it is ever forgiven — the result is a subset of `missedWeeks`, so it cannot exceed the games missed and needs no cap. And **byes need no special handling**: a bye week is not in the team's week set, so it can never reach `missedWeeks` and can never be excused, however long a reserve stint spanning it runs.

Pre-2016 seasons have no reserve feed to intersect, so they carry `seasonEndingAbsenceGames` instead; `injuryAdjustedFullSeasonDenominator` takes the larger of it and `excusedGames`, which are both counts of _games missed_, and caps at `|missedWeeks|`. In the shipped data the cap is currently unreachable on either path — `seasonEndingAbsenceGames` is itself computed over the same rest-adjusted team weeks (`scripts/update-data.ts`, `loadSnapData`) that bound `|missedWeeks|`, so it is already bounded by the quantity the cap compares against. It is kept as cheap defence-in-depth against a future signal that is not so bounded. We then subtract `excusedGames × (teamSeasonDen / gameCount)` where `gameCount` is the number of distinct games that franchise played in `snap_counts`. Applied only for single-franchise seasons when merging draft output (`resolveCumulativeLoadWithInjury`) — for a traded player, absence from one team's remaining schedule is a transaction, not an injury.

Consumers must read `Season.excusedGames` rather than re-derive anything from `injuryReportWeeks` or `reserveWeeks`: those are week counts and any re-derivation from them will disagree with the score being displayed (`src/lib/explainDraftScore.ts` reads the stored field for exactly this reason).

Note the **availability** term of the season score (`gamesPlayed / teamGames`, 30% weight) is untouched by any of this — including `reserveWeeks` — so a season-ending injury still costs a player most of that component; the adjustment only stops Load from reading the missed games as bench time.

**Known limitation.** `roster_weekly`'s status code (`status_description_abbr`) is what would separate injury reserve from other reserve (military, suspension, non-football illness), but it is only usably populated in 2020+; 2016–2019 carries a code on 3–5% of rows, effectively none. So for 2016–2019, `reserveWeeks` cannot distinguish injury from other reserve and forgives both alike. Where the code _is_ populated the known non-injury codes are excluded by name: `R62` and `R59` are both COVID-19 reserve (`NON_INJURY_RESERVE_CODES` in `src/lib/reserveWeeks.ts` carries the evidence — each is confined to 2020/2021, and the median `R59` stint is one week, shorter than the three-game injured-reserve minimum). An earlier version of this note bounded the 2016–2019 over-forgiveness at "roughly 15–25% of reserve weeks" by grouping `R01`+`R59` as injury; that grouping was wrong and was the source of a defect that inflated 2020 and 2021 forgiveness. The true bound is larger than 15–25% and is not restated as a number here until it is recomputed against `R01` alone. The exposure is still bounded by the non-injury share of coded reserve weeks rather than unlimited, and still strictly better than the status quo, which forgave those seasons at 0%.
...
**Implementation:** `scripts/update-data.ts`, `buildTeamSeasonDenominatorTotals`, `injuryAdjustedFullSeasonDenominator`, and `resolveCumulativeLoadWithInjury` in `src/lib/teamSeasonDenominator.ts`; the excusal rule in `src/lib/absenceWeeks.ts`; `seasonEndingAbsenceGames` in `src/lib/seasonEndingAbsence.ts`; `reserveWeeks` in `src/lib/reserveWeeks.ts`; per-game helpers in `src/lib/snapCountTotals.ts`. Stored as `cumulativeSnapShare` on each `Season`.
...
### 7.4b Free-agent expectation baseline

**Function:** `deriveFreeAgentBaseline` in `src/lib/deriveFreeAgentBaseline.ts`; consumed via `expectedScoreForFreeAgent` in `src/lib/overSlot.ts`; stored in `src/data/fa-baseline.json`.

Undrafted free agents have no draft slot to fit a curve against, so instead of §7.4's per-slot curve they are compared to one scalar: the cohort's own mean score.

- **Input population:** every free agent (§ Undrafted Free Agents in `SPEC_CLARIFICATIONS.md`), drawn from classes old enough to have completed their window. Every member of the cohort has already taken at least one snap — that is the cohort rule in `scripts/update-data.ts`, which drops any eligible player with no `firstSnapTeam`. The fit additionally skips a free agent with no played season row (`pickHasSeasonSnapData`, which means "has a played season row", not "took a snap"); for this population the two coincide, so the guard is a belt-and-braces check rather than the source of the snap restriction.
- **Maturity lag:** a class is included only if `class.year ≤ latestDraftClassYear − DRAFT_SLOT_MATURITY_LAG` (3) — the identical cutoff §7.4 uses for the pick-slot curve, so the two expectations reflect the same notion of "settled". The reference year is passed in by `scripts/derive-fa-baseline.ts`, read from the newest `draft-{year}.json`, **not** derived from the free-agent classes: those stop at the newest season anyone has played, while draft classes run one year further (the class drafted for the upcoming season). Deriving it from the free-agent classes lands the cutoff a year early and drops a debut class that has in fact played all three seasons of its window.
- **Scoring basis:** `getPlayerDraftScore(freeAgent, { draftingTeamOnly: true })` — drafting-team mode, matching how the pick-slot curve is fit, so a free agent's residual and a pick's residual are computed the same way.
- **Resulting scalar:** the mean of those scores. In the shipped data: `expected: 18.02`, fit over `playerCount: 1947` free agents from classes 2013–2023 (`matureFrom`/`matureTo`) — the same mature span the pick-slot curve is fit over.
- **Where it's stored:** `src/data/fa-baseline.json`, alongside `generatedAt`, `method`, `matureFrom`, `matureTo`, and `playerCount` for provenance. Regenerated by `pnpm update-data`, the same as the pick-slot curve.

```
expected(freeAgent) = fa-baseline.json.expected        (one scalar, not a curve)
overSlot(freeAgent) = score(freeAgent) − expected(freeAgent)
```

**Why one number and not a curve.** A draft slot varies continuously and predicts playing time on its own; an undrafted signing carries no equivalent ranking to interpolate over. Position baselines already normalize load before either score is computed, so what remains for the free-agent population is one group with one expectation.

**Why it is not a like-for-like bar with a pick's slot expectation.** §7.4's curve is fit over every drafted pick with a season row, including picks who never took a snap and score near zero — a draft slot is assigned regardless of outcome. This baseline is fit only over free agents who took at least one snap, because an undrafted player who never took one is not in the cohort at all and generates no season row to average in (a practice-squad player is on a roster with zero snaps and is likewise absent); there is no "never played" population to include. So 18.02 is a survivors' average and a pick's slot expectation is an everyone average. Both are 0–100 scores in the same units, but they answer different questions, and reading a free agent's over-slot value against a pick's as though they were interchangeable overstates what either one says. Compare a free agent's expectation to an early first-rounder's, not a seventh-rounder's, if the intuition "free agents are expected to do less" is what's wanted — the seventh-round comparison is depressed by picks who never played at all.

**Zero-centering.** Because 18.02 is this cohort's own mean, free-agent over-slot sums to (approximately) zero across the full cohort by construction. It ranks a team's undrafted signings against other teams' undrafted signings; it cannot say whether undrafted free agents as a whole are underused or overvalued league-wide.
