...
export interface FreeAgentClass {
  year: number;
  /**
   * Season the generating pipeline wrote season rows out to, when the file
   * came from one. Out-of-league rows through this year are elided on the wire
   * and rebuilt at parse time — see `src/lib/trailingSeasons.ts`.
   */
...
/**
 * How many games each franchise played in each season, regular plus
 * postseason. Generated by `scripts/update-data.ts`.
 *
 * Exists so the pipeline can stop writing a season row for every year a player
 * spends out of the league: those rows are pure function of his franchise and
 * the year, and are rebuilt at parse time from this table. See
 * `src/lib/trailingSeasons.ts`.
 */
export interface TeamGamesData {
  /** UTC calendar date `YYYY-MM-DD` when the table was written. */
  generatedAt: string;
  /**
   * Keyed by season. `max` is the deepest run any franchise made that year —
   * the fallback for a player no franchise entry covers, matching
   * `resolveTeamGamesDenominator`'s own last resort.
   */
  gamesBySeason: Record<
    string,
